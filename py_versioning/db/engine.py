...
class CreateVersion(DatabaseCore):
    def __init__(self, versioned_db_path, storage_db_path, table_name):
        DatabaseCore.__init__(self, versioned_db_path, storage_db_path, table_name)
        self.version = None
        last_version = self.get_latest_version()
        if last_version:
            new_list = self.get_table_list()
            new_json = json.dumps(new_list)
            old_list = json.loads(last_version[3])
...
class CheckVersion(DatabaseCore):
    def __init__(self, versioned_db_path, storage_db_path, table_name):
        DatabaseCore.__init__(self, versioned_db_path, storage_db_path, table_name)
        self.table_name = table_name
